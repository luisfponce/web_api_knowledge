...
def test_root():
    ...
    assert response.json() == {"Portfolio App": "This is a simple app using FastAPI and mariadb."}
...
def get_token():
    response = client.post("/api/v1/auth/login", json={"username": TEST_USER, "password": TEST_PSW})
    assert response.status_code == 200
